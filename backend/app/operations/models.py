...
from sqlalchemy import String, DateTime, ForeignKey, Text, Enum as SQLEnum, Numeric, JSON
...
class DamageReport(Base):
    ...
    rental_id: Mapped[uuid.UUID] = mapped_column(GUID(), ForeignKey("rentals.id"), index=True)
    asset_id: Mapped[uuid.UUID] = mapped_column(GUID(), ForeignKey("product_assets.id"), index=True)
...
class OfflineAction(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(GUID(), primary_key=True, default=uuid.uuid4)
    organization_id: Mapped[uuid.UUID] = mapped_column(GUID(), ForeignKey("organizations.id", ondelete="CASCADE"), index=True)
    action_id: Mapped[str] = mapped_column(String(255), unique=True, index=True)
    action_type: Mapped[str] = mapped_column(String(50))
    rental_id: Mapped[uuid.UUID] = mapped_column(GUID(), ForeignKey("rentals.id", ondelete="CASCADE"), index=True)
    ...
    created_by: Mapped[Optional[uuid.UUID]] = mapped_column(GUID(), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
